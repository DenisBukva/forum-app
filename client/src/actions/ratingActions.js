...
export const createRating = (data) => async (dispatch) => {
	try {
		const response = await axios.post('/api/rating/add', data, jwtConfig());
...
export const deleteQuestionRating = ({questionId, value}) => async (dispatch) => {
	try {
		await axios.delete(`/api/rating/question/${questionId}?value=${value}`, jwtConfig());
...
export const getQuestionRatings = (questionId) => async (dispatch) => {
	try {
		...
		const response = await axios.get(`/api/rating/question/${questionId}`, config);
